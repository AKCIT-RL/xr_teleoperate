...
public class WebRTCSignalingUnity : MonoBehaviour
{
    ...
    public CurvedScreen curvedScreen;
    ...
    void Start()
    {
        ...
        EnsureRemoteVideoRendererAssigned();
        StartCoroutine(WebRTCSetup());
    }

    private void EnsureRemoteVideoRendererAssigned()
    {
        if (remoteVideoRenderer != null)
            return;

        Debug.Log("🔎 remoteVideoRenderer not assigned - attempting automatic assignment...");

        // 1) If a CurvedScreen is assigned in the Inspector, use it; otherwise try to find one.
        var curved = curvedScreen != null ? curvedScreen : FindObjectOfType<CurvedScreen>();
        if (curved != null)
        {
            // The CurvedScreen script regenerates children at runtime; if the expected child
            // isn't present yet, wait a few frames for it to be created.
            var mono = curved.transform.Find("MonoScreen");
            if (mono == null)
            {
                StartCoroutine(WaitForCurvedChildrenAndAssign(curved));
                return;
            }

            if (mono != null)
            {
                var r = mono.GetComponent<Renderer>();
                if (r != null)
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/MonoScreen");
                    return;
                }
            }

            var left = curved.transform.Find("LeftScreen");
            if (left != null)
            {
                ...
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/LeftScreen");
                    return;
                }
            }

            var right = curved.transform.Find("RightScreen");
            if (right != null)
            {
                ...
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/RightScreen");
                    return;
                }
            }
        }

        // 2) Try to find GameObjects by common names created by CurvedScreen
        var go = GameObject.Find("MonoScreen") ?? GameObject.Find("LeftScreen") ?? GameObject.Find("RightScreen");
        if (go != null)
        {
            ...
            {
                remoteVideoRenderer = r;
                Debug.Log($"✅ remoteVideoRenderer auto-assigned from GameObject '{go.name}'");
                return;
            }
        }

        // 3) Fallback: find any Renderer whose name contains 'screen' (case-insensitive)
        var renderers = FindObjectsOfType<Renderer>();
        foreach (var rr in renderers)
        {
            if (rr.gameObject.name.IndexOf("screen", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                remoteVideoRenderer = rr;
                Debug.Log($"✅ remoteVideoRenderer auto-assigned from renderer '{rr.gameObject.name}'");
                return;
            }
        }

        Debug.LogWarning("⚠️ remoteVideoRenderer could not be auto-assigned. Assign a Renderer (e.g., MonoScreen) in the Inspector.");
    }

    private IEnumerator WaitForCurvedChildrenAndAssign(CurvedScreen cs)
    {
        ...
        while (frame < maxFrames)
        {
            ...
            var mono = cs.transform.Find("MonoScreen");
            if (mono != null)
            {
                var r = mono.GetComponent<Renderer>();
                if (r != null)
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/MonoScreen (delayed)");
                    yield break;
                }
            }

            var left = cs.transform.Find("LeftScreen");
            if (left != null)
            {
                var r = left.GetComponent<Renderer>();
                if (r != null)
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/LeftScreen (delayed)");
                    yield break;
                }
            }

            var right = cs.transform.Find("RightScreen");
            if (right != null)
            {
                var r = right.GetComponent<Renderer>();
                if (r != null)
                {
                    remoteVideoRenderer = r;
                    Debug.Log("✅ remoteVideoRenderer auto-assigned from CurvedScreen/RightScreen (delayed)");
                    yield break;
                }
            }
        ...
        Debug.LogWarning("⚠️ Waited for CurvedScreen children but none were created in time.");
    }
    ...
    private IEnumerator WebRTCSetup()
    {
        ...
        channel.OnMessage = msg =>
        {
            ...
            Debug.Log("📥 Python → Unity: " + text);

            if (trackerSender != null)
            {
                trackerSender.EnqueueIncomingMessage(text);
            }
        };
