...
class ImageClientVideoTrack(VideoStreamTrack):
    ...
    def set_stereo_mode(self, enabled: bool):
        # Toggled live by Unity's data channel (see "stereo_vision" messages
        # in handle_client below). Crops the exact pixel array here, not a
        # texture in Unity, so there's no ambiguity from decoder/texture
        # padding — Unity always just displays whatever frame arrives.
    ...
    async def recv(self):
        ...
        # Keep the binocular pair intact when Unity should render stereo
        # side-by-side; otherwise crop to the left eye only.
        if not self._stereo_mode and len(head_img.shape) == 3 and head_img.shape[1] >= 2 * head_img.shape[0]:
...
async def main():
    ...
    if args.turn_url:
        if not args.turn_username or not args.turn_password:
            raise ValueError("--turn-url requires --turn-username and --turn-password")
        ice_servers.append(
            RTCIceServer(
                urls=[args.turn_url],
